"""
Final timetable solver with strict hour enforcement and logging
"""
...
class FinalTimetableSolver:
    """Final solver with strict hour enforcement"""
    ...
    def __init__(
        ...
        # Parse subjects with strict hour reading
        self.subjects = []
        for s in subjects:
            hours = int(s.get("hours_per_week", 0))
            subject = Subject(s["name"], s["type"], hours)
            self.subjects.append(subject)
            logger.info(f"Subject loaded: {subject.name}, Type: {subject.type}, Hours: {subject.hours}")
        ...
        logger.info(f"Solver initialized with {len(self.subjects)} subjects, {len(self.rooms)} rooms, {len(self.days)} days")

    def solve(self) -> Optional[List[Dict[str, Any]]]:
        """Solve with STRICT hour constraints"""
        logger.info("=" * 50)
        logger.info("STARTING FINAL TIMETABLE SOLVER")
        logger.info("=" * 50)
        ...
        logger.info("Variables created successfully")
        ...
        logger.info("Teacher overlap constraint added")
        ...
        logger.info("Room type constraint added")

        # Constraint 4: STRICT HOUR CONSTRAINTS - THIS IS THE KEY
        logger.info("=" * 50)
        logger.info("ADDING HOUR CONSTRAINTS (STRICT)")
        logger.info("=" * 50)

        for subject in self.subjects:
            logger.info(f"\nProcessing subject: {subject.name}")
            logger.info(f"  Type: {subject.type}")
            logger.info(f"  Required hours: {subject.hours}")

            if subject.type == "theory":
                # Theory only - all slots must be in classrooms
                all_slots = []
                for day in self.days:
                    for slot in self.slots:
                        if subject.name in x[day][slot]:
                ...
                if all_slots:
                    ...
                    logger.info(f"  ✓ Added constraint: sum of all slots == {subject.hours}")
                else:
                    logger.warning(f"  ✗ No slots found for theory subject {subject.name}")

            elif subject.type == "practical":
                # Practical only - all slots must be in labs
                all_slots = []
                for day in self.days:
                    for slot in self.slots:
                        if subject.name in x[day][slot]:
                ...
                if all_slots:
                    ...
                    logger.warning(f"  ✗ No slots found for practical subject {subject.name}")
                ...
                logger.info(f"  Splitting: {theory_hours} theory + {practical_hours} practical")
                ...
                if theory_slots:
                    ...
                    logger.info(f"  ✓ Added constraint: theory slots == {theory_hours}")
                else:
                    logger.warning(f"  ✗ No theory slots found")
                ...
                if practical_slots:
                    ...
                    logger.info(f"  ✓ Added constraint: practical slots == {practical_hours}")
                else:
                    logger.warning(f"  ✗ No practical slots found")

        logger.info("\n" + "=" * 50)
        logger.info("HOUR CONSTRAINTS COMPLETE")
        logger.info("=" * 50)
        ...
        logger.info("Practical day constraint added")
        ...
        logger.info("Mix rule constraint added")
        ...
        logger.info("Practical subjects per day constraint added")
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            solution = self._extract_solution(solver, x)
            logger.info(f"Generated {len(solution)} timetable slots")
            return solution
        else:
    ...
    def _extract_solution(self, solver, x) -> List[Dict[str, Any]]:
        """Extract solution with verification"""
        solution = []
        hours_count = {}
                            ...
                            if teacher.name in x[day][slot][subject.name]:
                                for room in self.rooms:
                                    if solver.Value(x[day][slot][subject.name][teacher.name][room.name]) == 1:
                                        ...
                                        # Count hours
                                        if subject.name not in hours_count:
                                            hours_count[subject.name] = 0
                                        hours_count[subject.name] += 1
        ...
        logger.info("=" * 50)
        for subject in self.subjects:
            actual = hours_count.get(subject.name, 0)
            expected = subject.hours
            status = "✓" if actual == expected else "✗"
            logger.info(f"{status} {subject.name}: Expected {expected}h, Got {actual}h")
        logger.info("=" * 50)
