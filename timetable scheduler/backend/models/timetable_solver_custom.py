"""
Custom timetable solver using OR-Tools CP-SAT
Adapted for the academic timetable system
"""
...
class Subject:
    def __init__(self, name: str, subject_type: str, hours: int):
        self.name = name
        self.type = subject_type  # "theory", "practical", "both"
        self.hours = hours
...
class Room:
    def __init__(self, name: str, room_type: str):
        self.name = name
        self.type = room_type  # "classroom" or "lab"


class CustomTimetableSolver:
    """
    Custom solver for timetable generation with advanced constraints
    """
    ...
    def __init__(
        ...
        self.slots = [1, 2, 3, 4, 5, 6, 7]  # 9-10, 10-11, 11-12, 12-1, 2-3, 3-4, 4-5
        self.time_mapping = {
            1: "09:00-10:00",
            2: "10:00-11:00",
            3: "11:00-12:00",
        ...
        # Parse subjects and create teachers
        ...
        # Shuffled orders for search diversity
        ...
        for teacher in self.teachers:
            ...
            if subject_name not in self.subject_teachers:
                self.subject_teachers[subject_name] = []
            self.subject_teachers[subject_name].append(teacher.name)

    def solve(self) -> Optional[List[Dict[str, Any]]]:
        """Solve the timetable"""
        logger.info("Starting custom timetable solver...")
        ...
        # Constraint 1: Teacher cannot teach two sessions at once
        for day in self.days:
            for slot in self.slots:
                for teacher in self.teachers:
                    teacher_assignments = []
        ...
        # Constraint 2: Room cannot host multiple sessions at once
        for day in self.days:
            for slot in self.slots:
                for room in self.rooms:
                    room_assignments = []
        ...
        # Constraint 4: Weekly hours per subject
        y = {}
        for subject in self.subjects:
            theory_terms = []
            practical_terms = []
                ...
                for slot in self.slots:
                    if subject.name in x[day][slot]:
                        for teacher in self.teachers:
                            if teacher.name in x[day][slot][subject.name]:
                                for room in self.classrooms:
                                    theory_terms.append(x[day][slot][subject.name][teacher.name][room.name])
                                for room in self.labs:
                                    practical_terms.append(x[day][slot][subject.name][teacher.name][room.name])

            if subject.type == "theory":
                if theory_terms:
                    model.Add(sum(theory_terms) == subject.hours)
            elif subject.type == "practical":
                if practical_terms:
                    model.Add(sum(practical_terms) == subject.hours)
            elif subject.type == "both":
                theory_hours = max(1, subject.hours // 2)
                practical_hours = subject.hours - theory_hours

                if theory_terms:
                    model.Add(sum(theory_terms) == theory_hours)
                if practical_terms:
                    model.Add(sum(practical_terms) == practical_hours)

                # Choose one day for practicals
                y[subject.name] = {}
                for day in self.days:
                    ...
                    for slot in self.slots:
                        if subject.name in x[day][slot]:
                            for teacher in self.teachers:
                                if teacher.name in x[day][slot][subject.name]:
                                    for room in self.labs:
                                        day_lab_terms.append(x[day][slot][subject.name][teacher.name][room.name])
                    if day_lab_terms:
                        model.Add(sum(day_lab_terms) == practical_hours).OnlyEnforceIf(y[subject.name][day])
                        model.Add(sum(day_lab_terms) == 0).OnlyEnforceIf(y[subject.name][day].Not())

                model.Add(sum(y[subject.name][d] for d in self.days) == 1)

        # Constraint 5: Hard mix rule - if practical on day, must have theory
        ...
        # Constraint 6: At most 2 practical subjects per day
        for day in self.days:
            chosen_for_day = []
            for subject in self.subjects:
        ...
        # Objective: Maximize utilization with preferences
        objective_terms = []
        for day in self.days:
            for slot in self.slots:
                for subject in self.subjects:
